...
export class Conta {
    ...
    #validarTipo(tipo) {
        ...
        if (!tiposPermitidos.includes(tipo)) throw new AppError('Tipo deve ser corrente, poupanca ou investimento', 400)
        return tipo
    }

    podeSerDeletada() {
        if (this.#saldo.valor > 0) throw new AppError('Conta não pode ser deletada pois ainda tem saldo', 400)
        return true
    }
